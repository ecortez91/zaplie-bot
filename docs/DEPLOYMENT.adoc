...
| Upper bound in sats for a single person-to-person zap or self-issued invoice
  through the portal's LNbits gateway (default 1000000). Separate from
  `REWARDS_MAX_AMOUNT_SATS`, which caps automated rewards only. A malformed
  value fails closed: the portal backend refuses to start
| Portal backend
