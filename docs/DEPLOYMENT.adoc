...
The bot keeps mutable state (the weekly allowance run guard and the durable
Zap ledger `bot-zap-ledger.json`) under `ZAPLIE_DATA_DIR`. Bicep sets
...
is uncertain. After an unclean shutdown, stop/drain every bot process, inspect
the canonical store, reconcile any `processing` or `unknown` record with
LNbits, and only then remove an orphaned lock. Store corruption, an unreadable
data directory, or lock acquisition failure prevents payment.
...
| `ZAPLIE_DATA_DIR`
| Absolute persistent directory for the bot's mutable stores (weekly allowance
  run guard and `bot-zap-ledger.json`); Bicep sets `D:\home\data\zaplie` on
  the App Service, local development defaults to `.zaplie-data`
| Bot
