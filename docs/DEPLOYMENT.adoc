...
| Sats granted to each user's Allowance wallet when it is created
| Bot
...
| `show leaderboard` window in days (whole number, at least `1`, default
  `7` like the portal) and rows shown (default `10`); an invalid value
  makes the command reply "unavailable" and log the variable name
| Bot
...
| Durable directory for the mutable JSON stores, set on *both* App Services.
  ...
  `data.json` honours it as of this change; the bot's zap ledger honours it
  in the durable bot-state work, and the portal's remaining stores
  (identities, connections, pending rewards, secrets, webhook keys, GitHub
  App credentials) follow with the portal work.
...
. *Assign the admin app role.* The role is declared in `aad.manifest.json`,
  ...
  carries this role.
. *Prepare LNbits.* No extension is needed today (recurring allowance funding
  through the LNbits allowance extension is tracked separately): the bot
  provisions users through
  the core Users API, which requires an LNbits admin account. Create the
  treasury wallet that funds automation rewards and copy its admin key into
  `REWARDS_TREASURY_ADMINKEY`.
